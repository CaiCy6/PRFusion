...
class ImageFusionBlock(nn.Module):
    def __init__(self, in_channels, out_channels, layer_id, hidden_rate=4, init_mode='fancy', key_norm=False):
        super().__init__()
        self.layer_id = layer_id
        self.ln1 = nn.LayerNorm(in_channels)
        self.AdConv1 = AdaptiveDepthwiseSeparableConv(in_channels, out_channels)  # First adaptive depthwise conv
        self.AdConv2 = AdaptiveDepthwiseSeparableConv(out_channels, out_channels)  # Second adaptive depthwise conv
        self.att = VRWKV_SpatialMix_Tri_Eff_2D(out_channels, 12, layer_id, init_mode, key_norm=key_norm)
        self.gamma1 = nn.Parameter(torch.ones((out_channels)), requires_grad=True)
        self.ln2 = nn.LayerNorm(out_channels)
        self.ffn = VRWKV_ChannelMix_2D(out_channels, 12, layer_id, hidden_rate, init_mode, key_norm=key_norm)
        self.gamma2 = nn.Parameter(torch.ones((out_channels)), requires_grad=True)

    def forward(self, x1, x2):
        b, c, h, w = x1.shape  # Assuming both inputs have the same size

        # First stage: fusion of inputs and downsampling
        x1 = self.AdConv1(x1)
        x2 = self.AdConv1(x2)
        ...
        # Fusion step: merging both inputs
        x = torch.cat((x1, x2), dim=-1)  # Concatenate both inputs
        x = x + self.gamma1 * self.att(self.ln1(x), (h, w))
        x = rearrange(x, 'b (h w) c -> b c h w', h=h, w=w)
        
        # Second stage: applying second adaptive convolution
        x = self.AdConv2(x)
        x = rearrange(x, 'b c h w -> b (h w) c')
        x = x + self.gamma2 * self.ffn(self.ln2(x), (h, w))
        x = rearrange(x, 'b (h w) c -> b c h w', h=h, w=w)

        # Adding residual connection
        x1_residual = self.AdConv1(x1_residual)
        x2_residual = self.AdConv1(x2_residual)
        
        return x + x1_residual + x2_residual
